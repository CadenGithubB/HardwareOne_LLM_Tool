...
def main() -> None:
    ...
    if extra_tokens:
        print(f"  (+{len(extra_tokens)} extra special tokens from {args.special_tokens})")
    ...
    # ── Token coverage audit (fail fast, BEFORE training) ─────────────────
    # See training_scripts/audit_token_coverage.py: a query-key term is usable
    # on-device only if its bare or space-prefixed form is a single token.
    try:
    ...
    if args.text:
        ...
        def pack_qa_blocks(examples):
            ...
            for ids in examples["input_ids"]:
                ...
                # Train an explicit stop: append one UNMASKED EOS after the
                # answer so the model learns to halt instead of rambling.
    ...
    print(f"Dataset: {len(train_ds):,} blocks of {args.seq_len} tokens")
